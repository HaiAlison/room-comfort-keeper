...
export const Route =
  createFileRoute("/_app/dashboard")({
    head: () => ({
      meta: [
        {
          title:
            "Dashboard — ThermaGuard Room Monitoring",
        },
        {
          name: "description",
          content:
            "Room temperature, history, threshold and alerts.",
        },
      ],
    }),
...
function DashboardPage() {
  ...
  // Chỉ Monitoring mới quyết định
  // Dashboard có loading/error hay không
  const loading =
    current.isLoading ||
    threshold.isLoading;
      ...
      <div className="grid gap-4 lg:grid-cols-3">
        <Card className="rounded-2xl shadow-[var(--shadow-soft)] lg:col-span-2">
          <CardHeader className="grid grid-cols-[minmax(0,1fr)_auto] items-center gap-3">
            <div className="min-w-0">
              ...
              <CardDescription>
                Last 24 hours vs configured thresholds
              </CardDescription>
            </div>
        ...
        <div className="grid gap-4">
          ...
          <StatisticCard
            ...
            icon={Gauge}
            tone="muted"
            hint="Adjust temperature thresholds"
            footer={
              <Button
                asChild
                size="sm"
